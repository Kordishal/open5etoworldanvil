...
def get_content(world_id: str, category_id: str) -> Iterable[Dict[str, Any]]:
    backgrounds = requests.get('https://api.open5e.com/v1/backgrounds/')
    results = backgrounds.json()['results']

    for result in results:
        sidepanelcontenttop = ''
        if result['skill_proficiencies'] is not None:
            sidepanelcontenttop += f"--Skill Proficiencies::{result['skill_proficiencies'].strip()}--\n"
        if result['tool_proficiencies'] is not None:
            sidepanelcontenttop += f"--Tool Proficiencies::{result['tool_proficiencies'].strip()}--\n"
        if result['languages'] is not None:
            sidepanelcontenttop += f"--Languages::{result['languages'].strip()}--\n"
        if result['equipment'] is not None:
            sidepanelcontenttop += f"--Equipment::{result['equipment'].strip()}--\n"
        ...
        yield {
            ...
            'isDraft': False,
            'isWip': False,
            'content': to_bbcode(result['desc']).strip() + f"""
[h1]Features: {result['feature']}[/h1]
{to_bbcode(result['feature_desc']).strip()}

[h1]Suggested Characteristics[/h1]
{to_bbcode(result['suggested_characteristics']).strip()}""",
